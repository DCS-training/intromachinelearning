...
# Load both datasets
white_wine <- read_csv("../data/winequality-white.csv")
...
set.seed(42)
train_index <- createDataPartition(y_white, p = 0.8, list = FALSE)
...
linear_reg <- lm(y_train_white ~ ., data = X_train_white)
```

```{r}
# EXERCISE: Predict the quality scores on X_test using the trained model.
y_pred <- predict(linear_reg, newdata = X_test_white)
```
...
y_pred_red <- predict(linear_reg, newdata = X_red)
